...
contract DeLott is Ownable, IDeLott, ReentrancyGuard {
    ...
    uint256 public minPriceTicketInCake = 0.05 ether;
    ...
    function buyTickets(uint256 _lotteryId, uint32[] calldata _ticketNumbers)
        ...
        nonReentrant
    {
        require(_ticketNumbers.length != 0, "No tickets");
        require(
        ...
        //Calculate number of Cake transfer to this contract
        uint256 amountOfCakeToTransfer = _calculateTotalPriceforBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInCake,
            _ticketNumbers.length
        );

        //Transfer Cake from user to this contract
        cakeToken.safeTransferFrom(
            msg.sender,
            address(this),
            amountOfCakeToTransfer
        );

        //Increment amountCollectedInCake
        _lotteries[_lotteryId].amountCollectedInCake += amountOfCakeToTransfer;

        for (uint256 i; i < _ticketNumbers.length; ) {
            uint32 thisTicketNumber = uint32(_ticketNumbers[i]);

            require(
                (thisTicketNumber >= 100000) && (thisTicketNumber <= 1999999),
            ...
            // Increment the number of tickets number
            currentTicketId++;
        ...
        emit TicketPurchase(msg.sender, _lotteryId, _ticketNumbers.length);
    }
    ...
    function claimTickets(
        ...
        // Initialize the rewardInCakeToTransfer
        uint256 rewardInCakeToTransfer = 0;

        for (uint256 i; i < _ticketIds.length; i++) {
            require(_brackets[i] < 6, "Bracket out of range"); // 0-5
            ...
            require(msg.sender == _tickets[thisTicketId].owner, "Not owner");
            ...
            // Increment the rewardInCakeToTransfer
            rewardInCakeToTransfer += rewardForTicketId;
        }

        // Transfer the rewardInCakeToTransfer to the user
    ...
    function closeLottery(uint256 _lotteryId)
        ...
        nonReentrant
    {
        require(_lotteries[_lotteryId].status == Status.Open, "Not open");
        require(block.timestamp > _lotteries[_lotteryId].endTime, "Not ended");
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        require(_lotteries[_lotteryId].status == Status.Closed, "Not closed");
        require(
            _lotteryId == randomGenerator.viewLatestLotteryId(),
            "Numbers not drawn"
        );

        // Calculate the final number based on randomResult generated by ChainLink fallback
        uint32 finalNumber = randomGenerator.viewRandomResult();

        // Initialize a number to count addresses in the previous bracket
        uint256 numberAddressesInPreviousBracket;

        // Calculate the amount to share post-treasury
        uint256 amountToShareToWinners = (_lotteries[_lotteryId]
            .amountCollectedInCake *
            (10000 - _lotteries[_lotteryId].treasuryFee)) / 10000;

        // Initialize the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in CAKE for each bracket by starting from the highest one
        for (uint32 i = 6; i > 0; ) {
            // uint32 j = 5 - i;
            uint32 transformedWinningNumber = _bracketCalculator[i - 1] +
                (finalNumber % (uint32(10)**(i)));
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[i - 1] != 0) {
                    _lotteries[_lotteryId].cakePerBracket[i - 1] =
                        ((_lotteries[_lotteryId].rewardsBreakdown[i - 1] *
                ...
                _lotteries[_lotteryId].cakePerBracket[i - 1] = 0;
    ...
    function startLottery(
        ...
        require(
            _rewardsBreakdown[0] +
                ...
                _rewardsBreakdown[5] ==
                10000,
            "Rewards breakdown must sum to 10,000"
        );
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(cakeToken), "Cannot be CAKE");
    ...
    function setMaxNumberTicketsPerBuy(uint256 _maxNumberTicketsPerBuy)
        ...
        onlyOwner
    {
        require(_maxNumberTicketsPerBuy > 0, "Invalid max number of tickets");
    ...
    function setOperatorAndTreasuryAndInjectorAddress(
        ...
        require(_injectorAddress != address(0), "Invalid injector address");
    ...
    function viewRewardsForTicketId(
        ...
        //Check lottery is in claimable state
        ...
        )
    {
        ...
        for (uint256 i; i < length; i++) {
            ...
            if (_tickets[lotteryTicketIds[i]].owner == address(0)) {
                ...
                // Ticket is not claimed (includes the ones that cannot be claimed)
                ticketStatuses[i] = false;
            }
        }
    ...
    function _calculateRewardsForTicketId(
        ...
        // Confirm that the two transformed numbers match, if not draw
